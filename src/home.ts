...
export function registerHomeRoutes(app: Express, deps: { dashboardEnabled: boolean; authMode: string }) {
  app.get("/", (req: Request, res: Response) => {
    const origin = getOrigin(req);
    const mcpUrl = `${origin}/mcp`;

    const dashboardHref = deps.dashboardEnabled ? "/dashboard" : "#";
    const dashboardDisabledAttr = deps.dashboardEnabled ? "" : ' aria-disabled="true"';
      ...
      <div class="topbar-inner">
        <div class="brand">
          ...
          <div class="workspace" title="Auth mode">
            <span class="mono">${escapeHtml(deps.authMode)}</span>
          </div>
        </div>
          ...
          <div class="hero-grid">
            ...
            <aside class="hero-aside" aria-label="Endpoints">
              <div class="spec">
                ...
                <div class="row"><div class="key">Dashboard</div><div class="val">${deps.dashboardEnabled ? "Enabled" : "Disabled"}</div></div>
              </div>
              <div class="links" aria-label="Quick links">
                <a href="/health">/health</a>
                ${deps.dashboardEnabled ? `<a href="/dashboard">/dashboard</a>` : ""}
              </div>
            </aside>
          </div>
        </section>
